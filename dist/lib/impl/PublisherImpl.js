 ...
 * of this to use
 */
class PublisherImpl extends events_1.EventEmitter {
    ...
    handleSubscriberConnection(socket, name, header) {
        ...
        socket.on('close', () => {
            this._log.info('Publisher client socket %s on topic %s disconnected', name, this.getTopic());
            socket.removeAllListeners();
            delete this._subClients[name];
            this.emit('disconnect');
        });
        socket.on('end', () => {
            this._log.info('Publisher client socket %s on topic %s ended the connection', name, this.getTopic());
        });
        socket.on('error', (err) => {
            this._log.warn('Publisher client socket %s on topic %s had error: %s', name, this.getTopic(), err);
        });
    ...
    addUdpSubscriber(connId, host, port, dgramSize) {
    ...
    _register() {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                const resp = yield this._nodeHandle.registerPublisher(this._topic, this._type);
...
exports.default = PublisherImpl;
//# sourceMappingURL=PublisherImpl.js.map